...
class CheckDependencesState(smach.State):
    ...
    def __check_params(self, params_name):
        if rospy.get_param(params_name):  # None
            self._print_info("Checking params '%s': OK" % params_name)
            return 'succeeded'
        else:
            self.ALL_OK = False
            self._print_fatal("Checking params '%s': Failed" % params_name)
            return 'aborted'
